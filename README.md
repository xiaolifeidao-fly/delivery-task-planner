...
The app-api base URL is baked into `delivery_bridge/remote_worker.py` as `DEFAULT_COMMAND_API_URL` (`http://47.110.3.214:10002/api`), the same way `server.py` fixes the task board address; the bridge starts one background Worker there using the current console credential stored by `/v1/session/heartbeat`. `DELIVERY_COMMAND_API_URL` (or `--command-api-url`) still overrides it for another deployment, and setting either to `off` disables the Worker while every loopback bridge route behaves as before. A missing value no longer disables anything: a forgotten variable used to leave the phone showing "未登记执行电脑" with a healthy-looking console and nothing but one line in the log.

The Worker registers as soon as the bridge starts, even before any project is bound: it asks `GET /spaces` for the business lines the signed-in account can write and registers an empty program list in each one, so "the plugin is not running" and "the plugin is running but no workspace is bound" stop looking identical on the phone. A registration or heartbeat failure is isolated per business line — one deleted project in the local mapping used to abort the whole round and take every other business line offline with it.

When a local bridge request validates a program and workspace, the bridge records the authoritative program business line and the local `(programId, workspace)` mapping under the runtime directory with mode `0600`. The Worker registers only the mapped program IDs with app-api, never an absolute local path. It performs long-poll claims, lease-backed activity updates, heartbeats and terminal completion. Restarted Workers re-register and the command service re-dispatches an expired lease according to its two-minute recovery policy.

The Worker accepts only `business.conversation`, `task.execute`, `task.execute-batch`, `task.execute-sequence`, `task.conversation`, `task.stop`, `task.stop-all`, `documents.cloud-sync`, and the fixed `git.*` command set documented in [`../../doc/module/remote-worker-4971a295d5/design/remote-command-worker.md`](../../doc/module/remote-worker-4971a295d5/design/remote-command-worker.md). It cannot run a shell command, import an arbitrary Python method, or use a project workspace carried by the remote command; a `business.conversation` command carries a logical business workspace name only, which the Worker resolves beneath its own controlled business root. Before activity, result and error payloads return to app-api, the Worker removes local absolute paths; task completion also performs the existing cloud-document sync when that project enables it.

## Remote business interview mode

Business interviews reach the Worker as `business.conversation` commands, so the Go service no longer needs a route to this machine. The command carries the logical business workspace name only; the Worker resolves it under its own business root, downloads any app-api-held attachments into that workspace, and reports the running turn's conversation snapshot back as command activity until it finishes. Set `business.kodes.transport=command` on `web-api` to use it. The loopback route below stays available for a deployment that still runs a dedicated, reachable interview host (`business.kodes.transport=http`).
...
- `remote_worker.py` owns app-api polling, stable Worker identity, the local workspace mapping store, fixed command dispatch, leases, activity/result sanitization and cancellation handoff.
...
- `restart_helper.py` preserves the bridge command-line arguments and restarts the bridge after the HTTP response is delivered. On macOS it hands control back to the per-user LaunchAgent. On Linux `scripts/start_http.sh` installs a `systemd --user` unit (`delivery-task-planner-bridge.service`, `Restart=always`, lingering enabled when permitted) so the bridge survives logout and reboot the way the LaunchAgent does, and the helper restarts that unit instead of spawning a second bridge on port 8765; a host without a user systemd falls back to the previous `nohup` launch and says so. On Windows the logon task runs `windows_supervisor.py`, which relaunches a stopped bridge worker in under a second instead of waiting for Task Scheduler's one-minute failure interval; an update also migrates an older direct-bridge task to the supervisor action and waits for `/healthz` before declaring the restart usable. Restart diagnostics are written to the platform runtime directory, and a stale `restarting` job becomes retryable after 45 seconds instead of polling forever.
