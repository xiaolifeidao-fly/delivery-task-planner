...
Set `DELIVERY_COMMAND_API_URL` to the deployed `server/app-api` base URL before starting the bridge, for example `DELIVERY_COMMAND_API_URL=https://app-api.example.com ./scripts/start_http.sh`. The bridge then starts one background Worker using the current console credential stored by `/v1/session/heartbeat`. With no URL configured, the Worker remains disabled and every existing loopback bridge route behaves as before.
