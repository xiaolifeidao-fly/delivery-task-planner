...
An installation refreshes the Codex personal-marketplace source and invokes `codex plugin add`. Existing Claude Code installations are copied into a new versioned cache directory and `installed_plugins.json` is replaced atomically, leaving the previous cache available to already running Claude sessions. Skill-only releases become available to new sessions immediately. When runtime files change, a server-side monitor waits until delivery runs are idle and restarts the bridge automatically; it continues even if the browser tab closes. `GET /v1/plugin/info` exposes the running plugin version, and `GET /v1/plugin/runtime-test` reads a hard-coded value from the live Python process for restart verification. Backups and update state live under `~/.local/state/delivery-task-planner/`.
